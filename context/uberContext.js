...
export const UberProvider = ({ children }) => {
  ...
  const [recipientAddressGlobal, setRecipientAddressGlobal] = useState('');
  let metamask
      ...
      if (!data.data || data.data.length === 0) {
        MySwal.fire({
          ...
          if (result.isConfirmed && result.value) {
            requestToCreateUserOnSanity(walletAddress, result.value);
  ...
  const disconnectWallet = () => {
    ...
    // Optional: Show alert to the user to manually disconnect from Metamask
    MySwal.fire({
      title: 'Desconectado',
      text: 'Por favor, desconecte manualmente su cuenta de Metamask.',
      icon: 'info',
  ...
  const requestToCreateUserOnSanity = async (address, name) => {
    try {
  ...
  return (
    <UberContext.Provider value={{
      ...
      basePrice,
      metamask,
      recipientAddressGlobal, 
      setRecipientAddressGlobal,
